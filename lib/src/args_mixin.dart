...
mixin ArgsMixin {
  final List<String> _args = CommandCli.arguments;
...
List<String> _getArgs() {
  var args = List.of(CommandCli.arguments);
  _removeDefaultArgs(args);
...
int _getIndexArg(String arg) {
  return CommandCli.arguments.indexWhere((element) => element == arg);
}

String _getArg(String arg) {
  var index = _getIndexArg(arg);
  if (index != -1) {
    if (index + 1 < CommandCli.arguments.length) {
      index++;
      return CommandCli.arguments[index];
    } else {
      throw Exception("the '$arg' argument is empty");
    }
  }
